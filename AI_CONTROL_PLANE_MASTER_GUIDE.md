...
- **API keys & tiers** (`engine/auth.py`) — `POST /v1/auth/api-keys` issues a key (`aegis_...`) hashed with SHA-256 before storage; the raw key is shown exactly once. Three configurable rate-limit tiers exist to demonstrate the mechanism: **Free** (60 rpm / 40k tpm / $5 budget), **Pro** (300 rpm / 200k tpm / $100 budget), **Enterprise** (2000 rpm / 2M tpm / $2000 budget). This is a technical demonstration, not a paid product — no billing is wired up. `GET /v1/plans` is public.
